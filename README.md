...
Render uchun:
- GitHub'ga rootdagi `requirements.txt`, `bot.py`, `render.yaml` fayllarini push qiling.
- Render Web Service yarating.
- Environment Variables:
  - `BOT_TOKEN`
  - `BOT_USERNAME`
  - `WEBAPP_URL` — Netlify frontend domeni, masalan `https://webappgarden.netlify.app/`
  - `CHANNEL_USERNAME` — masalan `@meva_garden`
- Start command: `python bot.py`

Netlify frontend uchun:
- Frontend URL `WEBAPP_URL` sifatida qo'ying.
- Backend URL ni [config.js](config.js) dagi `window.APP_CONFIG.backendUrl` ga yozing. Masalan: `https://your-backend-url.onrender.com`.
